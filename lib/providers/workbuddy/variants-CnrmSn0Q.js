...
var WorkBuddyUpstreamClient = class {
	...
	*/
	...
	/**
	* [本地补丁 2026-09-23] 每日签到：读状态 / 领取。
	*
	* 端点与请求形态抄自 WorkBuddy 桌面 App 的 app.asar（BackendProvider的
	* getCheckinStatus / claimDailyCheckin）：两者都是 POST 空 body，信封 {code, msg, data}，
	* code === 0 才算成功。已在容器内对 CN 个人账号实测。
	*
	* 注意两个端点语义不同：checkin-status 是**签到活动看板**（season/theme/
	* week_progress），活动未开放时整份文档都是默认值（active=false、today_checked_in=false），
	* 不能当作"今天签没签"的依据；"已签到"要看 daily-checkin 的结果。
	*
	* ⚠️ [2026-09-25 实测] `code 10001` **不是单一语义**，实测两种：
	*   - 个人账号：`HTTP 400 / 10001 / "今天已签到，请明天再来"`
	*   - 企业账号：`HTTP 400 / 10001 / "企业账号不支持该操作"`
	* 只按 code 判断会把企业账号误报成"已签到"（界面显示"已签"，用户以为签过了）。
	* 所以 10001 必须再看 msg 文案，且企业账号（凭据里有 enterpriseId）应当**直接短路**、
	* 不必打这趟注定 400 的请求。
	*/
	async fetchCheckinStatus(credential) {
		const response = await fetch(`${billingBase(credential)}/v2/billing/meter/checkin-status`, {
			method: "POST",
			headers: billingHeaders(credential),
			body: JSON.stringify({}),
			signal: AbortSignal.timeout(JSON_TIMEOUT_MS)
		...
		if (!response.ok || envelope.code !== 0) return {
			...
			message: typeof envelope.msg === "string" ? envelope.msg : ""
		};
		return {
			ok: true,
	...
	/** 执行今日签到：`code 0` = 刚领到；`10001` 需再按 msg 区分"已签"与"企业版不支持"。 */
	async claimDailyCheckin(credential) {
		// 企业账号不支持每日签到，上游对它固定回 400/10001；凭据里既然已写着 enterpriseId，
		// 就别打这趟注定失败的请求（也让 /status 的自动签到不必为一个必然失败去等一次超时）。
		...
		/**
		* 同一个 10001 两种语义，只能看文案：个人版「今天已签到，请明天再来」→ 已签；
		* 企业版「企业账号不支持该操作」→ 不支持。上面的 enterpriseId 短路已覆盖企业版，
		* 这里是防上游改分支或文案时的兜底 —— 宁可显示"不支持"也不要谎报"已签"。
		*/
		if (code === 10001) return {
			state: /不支持|not supported|unsupported/i.test(message) ? "unsupported" : "already-claimed",
			code,
			message
		};
		return {
...
const WORKBUDDY_AUTH_FILE_ENV = "WORKBUDDY1_AUTH_FILE";
...
/**
* [本地分支 2026-09-23] 两个**国区**账号，同时使用。
*
* 原插件的两个变体是「国区 + 国际版」；本分支换成两个同区账号，
* 每个变体自带 provider id（= 模型渠道名）、环境变量、状态文件名与路由，
* 所以 DSH 里会同时出现 workbuddy1 / workbuddy2 两个渠道。
* 共用的国区行为（billing 基地、enterpriseId 分支等）均按 region === "cn" 走。
*
* [本地分支 2026-09-24] 再加第三个国区渠道 workbuddy3 —— 企业版账号。
* 变体表是数据驱动的：加一条就多一个 provider id / 环境变量 / 状态文件名 / 路由，
* 宿主侧的 registerAdapter、状态路由、签到路由都是从这张表派生的。
* 企业版不需要额外分支代码：凭据里带 enterpriseId 时，请求头自动改发
* X-Enterprise-Id / X-Tenant-Id，额度自动改问
* /v2/billing/meter/get-enterprise-user-usage（个人版接口对企业号返回空账户表，
* 会被渲染成"0 积分"）。
* 注意三条变体的 desktopFilename 仍是同一个 workbuddy-desktop.info，
* 所以 WORKBUDDY{1,2,3}_AUTH_FILE 这三个环境变量是**承重**的：
* 不设就会三条渠道一起去读桌面 App 当前那一个登录。
*/
/**
 * [2026-09-25] 渠道清单从**注册表**生成：`$DSH_HOME/connect-channels.json`
 * （不存在时回落与旧版逐字相同的 1/2/3 + trae + qoder，见 channel-registry.js）。
 *
 * 槽位数量不再写死：注册表里有几条 workbuddy，这里就生成几条变体。
 * 每条变体的状态文件名（ownFilename / probeFilename / catalogFilename /
 * visibilityFilename）与路由路径全部由 id 推导，互不冲突。
 *
 * 空位安全的原因有两条，缺一不可：
 *   1. 凭据文件不存在时 `WorkBuddyCredentialStore.status()` 回 `signed-out`，不抛错；
 *   2. 空位的凭据文件由 compose 显式指向 `connect-auth/workbuddy{N}.json`（不存在），
 *      **绝不能让它回退到桌面 App 的路径** —— 那会变成"和账号 1 读同一份凭据"，
 *      看起来像登录了，实际是同一个号（这正是 `configuredAuthFile` 必须对
 *      未知变体返回 undefined 的原因）。
 * 另外空位会被 `setExternalHidden` 整体隐藏，所以它的兜底模型列表不会漏进模型选择器。
 */
function workbuddyVariantOf(entry) {
	...
	return {
		...
		appName: "WorkBuddy",
		region: "cn",
		env: `WORKBUDDY${n}_AUTH_FILE`,
		desktopFilename: "workbuddy-desktop.info",
		ownFilename: `.${entry.id}-auth.json`,
		probeFilename: `.${entry.id}-probe.json`,
		catalogFilename: `.${entry.id}-catalog.json`,
		visibilityFilename: `.${entry.id}-model-visibility.json`,
		statusPath: `/plugins/dsh-workbuddy-connect/${n}/status`,
		probePath: `/plugins/dsh-workbuddy-connect/${n}/probe`
	};
}
const WORKBUDDY_VARIANTS = readRegistry().channels.filter((entry) => entry.kind === "workbuddy").map(workbuddyVariantOf);
