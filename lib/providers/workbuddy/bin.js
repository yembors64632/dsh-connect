...
function fallbackCount(variant) {
	return variant.id === "workbuddy1" ? FALLBACK_WORKBUDDY_MODELS.length : FALLBACK_WORKBUDDY_AI_MODELS.length;
}
...
async function run(argv) {
	...
	const variant = variantFor(providerId ?? "workbuddy1");
	if (variant === void 0) {
		const available = WORKBUDDY_VARIANTS.map((v) => v.id).join(", ");
		process.stderr.write(available === ""
			? "dsh-workbuddy-connect: no WorkBuddy channels configured; add a channel first\n"
			: `dsh-workbuddy-connect: provider ${JSON.stringify(providerId ?? "workbuddy1")} not configured; select --provider from: ${available}\n`);
		return 1;
	}
